...
/// It affects the relative ordering of these changes and  the scope of this ordering. Additionally, this policy
/// introduces the concept of coherent change sets.
/// </summary>
/// <remarks>
/// This policy controls the ordering and scope of samples made available to the subscriber, but the subscriber
/// application must use the proper logic in reading samples to guarantee the requested behavior.
/// </remarks>
public sealed class PresentationQosPolicy : IEquatable<PresentationQosPolicy>
{
    #region Properties
    /// <summary>
    /// Gets or sets a value that specifies how the samples representing changes to data instances are presented to
    /// a subscribing application.
    /// </summary>
    public PresentationQosPolicyAccessScopeKind AccessScope { get; set; }

    /// <summary>
    /// Gets or sets a value indicating whether allow one or more changes to an instance be made available to an
    /// associated data reader as a single change or not. If a data reader does not receive the entire set of coherent
    /// changes made by a publisher, then none of the changes are made available. The semantics of coherent changes
    /// are similar in nature to those found in transactions provided by many relational databases.
    /// By default, CoherentAccess is <see langword="false" />.
    /// </summary>
