...
/// <summary>
/// This class is the abstract base class for all the DCPS objects that support QoS policies, a
/// listener and a status condition. That is, <see cref="DomainParticipant" />, <see cref="Topic" />,
/// <see cref="Publisher" />, <see cref="Subscriber" />, <see cref="DataWriter" /> and <see cref="DataReader" />.
/// </summary>
public abstract class Entity
{
    ...
    /// condition can then be added to a <see cref="WaitSet" /> so that the application can wait for specific
    /// status changes that affect the <see cref="Entity" />.
    /// </summary>
    public StatusCondition StatusCondition => GetStatusCondition();

    /// <summary>
    /// Gets the list of communication statuses in the <see cref="Entity" /> that are 'triggered'. That is, the list
    /// of statuses whose value has changed since the last time the application read the status.
    /// </summary>
    /// <remarks>
    /// <para>When the <see cref="Entity" /> is first created or if the entity is not enabled, all communication
    /// statuses are in the “un-triggered”.</para>
    /// <para>The statuses returned by the StatusChanges property refers to the status that are triggered on the
    /// <see cref="Entity" /> itself and does not include statuses that apply to contained entities.</para>
    /// </remarks>
    ...
    #region Methods
    /// <summary>
    /// Enables the <see cref="Entity" />. The enable operation is idempotent. Calling enable on an already
    /// enabled <see cref="Entity" /> returns <see cref="ReturnCode.Ok" /> and has no effect.
    /// </summary>
    /// <remarks>
    ...
    /// The default setting of <see cref="EntityFactoryQosPolicy" /> is such that, by default, it is not necessary
    /// to explicitly call enable on newly created entities.</para>
    /// <para>If an <see cref="Entity" /> has not yet been enabled, the following kinds of operations may be invoked
    /// on it:
    /// <list type="bullet">
...
#if NET7_0_OR_GREATER
    [SuppressUnmanagedCodeSecurity]
    [LibraryImport(MarshalHelper.API_DLL, EntryPoint = "Entity_Enable")]
    [UnmanagedCallConv(CallConvs = new[] { typeof(CallConvCdecl) })]
    public static partial ReturnCode Enable(IntPtr e);

    [SuppressUnmanagedCodeSecurity]
    [LibraryImport(MarshalHelper.API_DLL, EntryPoint = "Entity_GetStatusCondition")]
    [UnmanagedCallConv(CallConvs = new[] { typeof(CallConvCdecl) })]
    public static partial IntPtr GetStatusCondition(IntPtr e);

    [SuppressUnmanagedCodeSecurity]
    [LibraryImport(MarshalHelper.API_DLL, EntryPoint = "Entity_GetStatusChanges")]
    [UnmanagedCallConv(CallConvs = new[] { typeof(CallConvCdecl) })]
    public static partial StatusMask GetStatusChanges(IntPtr e);

    [SuppressUnmanagedCodeSecurity]
    [LibraryImport(MarshalHelper.API_DLL, EntryPoint = "Entity_GetInstanceHandle")]
    [UnmanagedCallConv(CallConvs = new[] { typeof(CallConvCdecl) })]
    public static partial int GetInstanceHandle(IntPtr e);
#else
    [SuppressUnmanagedCodeSecurity]
