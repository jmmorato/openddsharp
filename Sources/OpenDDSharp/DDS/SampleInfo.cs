...
/// </summary>
/// <remarks>
/// The SampleInfo structure contains information associated with each Sample. The <see cref="DataReader"/> read
/// and take operations return two vectors. One vector contains Sample(s) and the other contains SampleInfo(s).
/// There is a one-to-one correspondence between items in these two vectors. Each Sample is described by the
/// corresponding SampleInfo instance.
/// </remarks>
public sealed class SampleInfo : IEquatable<SampleInfo>
{
    ...
    /// The associated DataSample may not contain data if it this sample indicates a change in sample state
    /// (for example Alive -> Disposed).
    /// </summary>
    ...
    /// <summary>
    /// Gets a value indicating whether  the associated instance has/has not been seen before. ViewState indicates
    /// whether the <see cref="DataReader" /> has already seen samples for the most current generation
    ...
    /// <summary>
    /// Gets the number of times the instance has become 'Alive' after being automatically disposed due
    /// to no active writers.
    /// </summary>
    public int NoWritersGenerationCount { get; internal set; }

    /// <summary>
    /// Gets number of samples related to this instances that follow in the collection returned by the
    /// <see cref="DataReader" /> read or take operations.
    /// </summary>
    public int SampleRank { get; internal set; }

    /// <summary>
    /// Gets the generation difference of this sample and the most recent sample in the collection. GenerationRank
    /// indicates the generation difference (ie, the number of times the instance was disposed and became alive again)
    /// between this sample, and the most recent sample in the collection related to this instance.
    /// </summary>
    public int GenerationRank { get; internal set; }

    /// <summary>
    /// Gets the generation difference between this sample and the most recent sample. The AbsoluteGenerationRank
    /// indicates the generation difference (ie, the number of times the instance was disposed and became alive again)
