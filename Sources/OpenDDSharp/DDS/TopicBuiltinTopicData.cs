...
/// Each TopicBuiltinTopicData sample in a Domain represents a <see cref="Topic" /> in that Domain: a new
/// TopicBuiltinTopicData instance is created when a newly-added <see cref="Topic" />  is enabled.
/// However, the instance is not disposed when a <see cref="Topic" /> is deleted by its participant because a topic
/// lifecycle is tied to the lifecycle of a Domain, not to the lifecycle of an individual participant.
/// An updated TopicBuiltinTopicData sample is written each time a <see cref="Topic" />  modifies one or
/// more of its QoS policy values.
/// </remarks>
