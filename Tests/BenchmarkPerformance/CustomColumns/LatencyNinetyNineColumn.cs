...
internal class LatencyNinetyNineColumn : IColumn
{
    public static readonly string OutputFolder = Path.Combine(Path.GetTempPath(), nameof(LatencyNinetyNineColumn));

    /// <inheritdoc/>
    public string Id => nameof(LatencyNinetyNineColumn);

    /// <inheritdoc/>
    public string ColumnName => "Latency 99%";
    ...
    /// <inheritdoc/>
    public string Legend => "Sample latency 99% in milliseconds";
    ...
    public string GetValue(Summary summary, BenchmarkCase benchmarkCase)
    {
        ...
        var filename = Path.Combine(OutputFolder, $"{name}-latency-ninety-nine.{numInstances}.{numSamples}.{payloadSize}.txt");
