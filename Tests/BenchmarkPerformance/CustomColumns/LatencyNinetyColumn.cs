...
internal class LatencyNinetyColumn : IColumn
{
    public static readonly string OutputFolder = Path.Combine(Path.GetTempPath(), nameof(LatencyNinetyColumn));

    /// <inheritdoc/>
    public string Id => nameof(LatencyNinetyColumn);

    /// <inheritdoc/>
    public string ColumnName => "Latency 90%";
    ...
    /// <inheritdoc/>
    public string Legend => "Sample latency 90% in milliseconds";
    ...
    public string GetValue(Summary summary, BenchmarkCase benchmarkCase)
    {
        ...
        var filename = Path.Combine(OutputFolder, $"{name}-latency-ninety.{numInstances}.{numSamples}.{payloadSize}.txt");
