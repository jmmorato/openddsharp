...
internal class LatencyAverageColumn : IColumn
{
    public static readonly string OutputFolder = Path.Combine(Path.GetTempPath(), nameof(LatencyAverageColumn));

    /// <inheritdoc/>
    public string Id => nameof(LatencyAverageColumn);

    /// <inheritdoc/>
    public string ColumnName => "Latency Avg.";
    ...
    /// <inheritdoc/>
    public string Legend => "Sample latency average in milliseconds";
    ...
    public string GetValue(Summary summary, BenchmarkCase benchmarkCase)
    {
        ...
        var filename = Path.Combine(OutputFolder, $"{name}-latency-average.{numInstances}.{numSamples}.{payloadSize}.txt");
