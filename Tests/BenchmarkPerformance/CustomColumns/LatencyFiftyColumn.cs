...
internal class LatencyFiftyColumn : IColumn
{
    public static readonly string OutputFolder = Path.Combine(Path.GetTempPath(), nameof(LatencyFiftyColumn));

    /// <inheritdoc/>
    public string Id => nameof(LatencyFiftyColumn);

    /// <inheritdoc/>
    public string ColumnName => "Latency 50%";
    ...
    /// <inheritdoc/>
    public string Legend => "Sample latency 50% in milliseconds";
    ...
    public string GetValue(Summary summary, BenchmarkCase benchmarkCase)
    {
        ...
        var filename = Path.Combine(OutputFolder, $"{name}-latency-fifty.{numInstances}.{numSamples}.{payloadSize}.txt");
