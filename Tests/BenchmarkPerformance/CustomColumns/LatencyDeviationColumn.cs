...
internal class LatencyDeviationColumn : IColumn
{
    public static readonly string OutputFolder = Path.Combine(Path.GetTempPath(), nameof(LatencyDeviationColumn));

    /// <inheritdoc/>
    public string Id => nameof(LatencyDeviationColumn);

    /// <inheritdoc/>
    public string ColumnName => "Latency Std. Dev.";
    ...
    /// <inheritdoc/>
    public string Legend => "Sample latency standard deviation in milliseconds";
    ...
    public string GetValue(Summary summary, BenchmarkCase benchmarkCase)
    {
        ...
        var filename = Path.Combine(OutputFolder, $"{name}-latency-deviation.{numInstances}.{numSamples}.{payloadSize}.txt");
