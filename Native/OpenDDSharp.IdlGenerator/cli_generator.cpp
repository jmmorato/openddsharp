...
bool cli_generator::gen_enum(AST_Enum *node, UTL_ScopedName *name, const std::vector<AST_EnumVal *> &contents,
                             const char *repoid) {
...
bool cli_generator::gen_typedef(AST_Typedef *node, UTL_ScopedName *name, AST_Type *base, const char *repoid) {
  ...
  switch (base->node_type()) {
    case AST_Decl::NT_sequence: {
      AST_Sequence *seq_type = dynamic_cast<AST_Sequence*>(base);
      if (seq_type->base_type()->node_type() == AST_Decl::NT_enum) {
        std::cerr << "ERROR: List of enum types are not supported in .NET. Field name: " << node->full_name()
                  << std::endl;
        return false;
      }
    ...
    case AST_Decl::NT_array: {
      AST_Array *arr_type = dynamic_cast<AST_Array*>(base);
      if (arr_type->base_type()->node_type() == AST_Decl::NT_enum) {
        std::cerr << "ERROR: Array of enum types are not supported in .NET. Field name: " << node->full_name()
                  << std::endl;
        return false;
      }
...
bool cli_generator::gen_struct(AST_Structure *structure, UTL_ScopedName *name, const std::vector<AST_Field *> &fields,
...
bool cli_generator::gen_union(AST_Union *, UTL_ScopedName *name, const std::vector<AST_UnionBranch *> &, AST_Type *,
                              const char *) {
...
std::string cli_generator::implement_struct_constructor(const std::vector<AST_Field *> &fields, const std::string name,
...
std::string
cli_generator::implement_struct_properties(const std::vector<AST_Field *> &fields, const std::string scoped_name) {
  std::string ret("");
...
std::string cli_generator::implement_struct_to_native(const std::vector<AST_Field *> &fields, const std::string name,
                                                      const std::string scoped_name) {
  std::string ret("::");
...
std::string cli_generator::implement_struct_from_native(const std::vector<AST_Field *> &fields, const std::string name,
                                                        const std::string scoped_name) {
  std::string ret("void ");
...
std::string cli_generator::get_typedef_seq_to_native(AST_Typedef *typedef_type, std::string field_name) {
  ...
  AST_Sequence *seq_type = dynamic_cast<AST_Sequence*>(typedef_type->base_type());
  ...
  switch (seq_type->base_type()->node_type()) {
    ...
    case AST_Decl::NT_pre_defined: {
      AST_PredefinedType *predefined_type = dynamic_cast<AST_PredefinedType*>(seq_type->base_type());
      if (predefined_type->pt() != AST_PredefinedType::PT_longdouble) {
        ret.append("            ret.");
        ret.append(field_name);
        ret.append("[i] = ");
...
std::string cli_generator::get_typedef_array_to_native(AST_Typedef *typedef_type, std::string field_name) {
  ...
  AST_Array *arr_type = dynamic_cast<AST_Array*>(typedef_type->base_type());
  ...
  for (unsigned int i = 0; i < n_dims; i++) {
    ...
    ret.append("System::Math::Min(m_" + field_name + "->GetLength(" + std::to_string(i) + "), " +
               std::to_string(dims[i]->ev()->u.ulval) + ");\n");
  }

  for (unsigned int i = 0; i < n_dims; i++) {
    std::string dim = "i_dim" + std::to_string(i);
    ret.append(
        "        for (unsigned int " + dim + " = 0; " + dim + " < dim" + std::to_string(i) + "_" + field_name + "; " +
        dim + "++) {\n");
  }
  ...
  switch (arr_type->base_type()->node_type()) {
    ...
    case AST_Decl::NT_pre_defined: {
      AST_PredefinedType *predefined_type = dynamic_cast<AST_PredefinedType*>(arr_type->base_type());
      if (predefined_type->pt() != AST_PredefinedType::PT_longdouble) {
        ret.append("            ret." + field_name);
        for (unsigned int i = 0; i < n_dims; i++) {
          ret.append("[i_dim" + std::to_string(i) + "]");
...
std::string cli_generator::get_typedef_array_from_native(AST_Typedef *typedef_type, std::string field_name) {
  ...
  ret.append(
      "    m_" + field_name + " = gcnew array<" + get_cli_type(arr_type->base_type()) + ", " + std::to_string(n_dims) +
      ">(");
  ret.append(std::to_string(dims[0]->ev()->u.ulval));
  unsigned int i = 1;
  while (i < n_dims) {
  ...
  for (unsigned int i = 0; i < n_dims; i++) {
    std::string dim = "i_dim" + std::to_string(i);
    ret.append(
        "    for (unsigned int " + dim + " = 0; " + dim + " < " + std::to_string(dims[i]->ev()->u.ulval) + "; " + dim +
        "++) {\n");
  }
