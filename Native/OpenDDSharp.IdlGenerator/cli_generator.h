...
class cli_generator : public dds_generator {
public:
    cli_generator();

    bool gen_struct(AST_Structure *node, UTL_ScopedName *name, const std::vector<AST_Field *> &fields,
...
private:
    ...
    std::string implement_struct_to_native(const std::vector<AST_Field *> &fields, const std::string name,
                                           const std::string scoped_name);

    std::string implement_struct_from_native(const std::vector<AST_Field *> &fields, const std::string name,
                                             const std::string scoped_name);

    std::string get_cli_type(AST_Type *type);

    std::string get_cli_default_value(AST_Type *type);
