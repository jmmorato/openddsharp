...
public:
	...
	std::ostringstream header_, impl_;// , idl_, ws_config_, itl_, facets_header_, facets_impl_, lang_header_;
	ACE_CString header_name_, impl_name_;// , idl_name_, ws_config_name_, itl_name_, facets_header_name_, facets_impl_name_, lang_header_name_, output_dir_, tao_inc_pre_;
	ACE_CString output_dir_, tao_inc_pre_;
...
private:	
	...
	bool java_, suppress_idl_, suppress_typecode_,
		generate_wireshark_, generate_itl_, v8_, face_ts_, cppcli_, csharp_, cwrapper_;

	ACE_CString export_macro_, export_include_, versioning_name_, versioning_begin_, versioning_end_, pch_include_, java_arg_, seq_, project_name_;
