...
public:
	...
	virtual bool gen_const(UTL_ScopedName* name, bool nestedInInteface, AST_Constant* constant) = 0;

	virtual bool gen_enum(AST_Enum* node, UTL_ScopedName* name, const std::vector<AST_EnumVal*>& contents, const char* repoid)
	{
		return true;
	}
	...
	virtual bool gen_struct_fwd(UTL_ScopedName* name, AST_Type::SIZE_TYPE size)
	{
		return true;
	}
	...
	virtual bool gen_union_fwd(AST_UnionFwd* /*node*/, UTL_ScopedName* /*name*/, AST_Type::SIZE_TYPE /*size*/)
	{
		return true;
	}

	virtual bool gen_module(AST_Module* node) = 0;

	virtual bool gen_module_end() = 0;
